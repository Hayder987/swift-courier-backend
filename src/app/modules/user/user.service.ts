...
const changeUserStatus = async (
	...
	if (existingUser.isDeleted || existingUser.status === UserStatus.DELETED) {
		throw new AppError(httpStatus.BAD_REQUEST, "Deleted user cannot be activated or suspended.");
	}

	// Prevent same status update
	if (existingUser.status === newStatus) {
		throw new AppError(httpStatus.BAD_REQUEST, `User is already ${newStatus.toLowerCase()}.`);
	}

	const now = new Date();

	const suspensionExpiresAt = new Date(now.getTime() + 15 * 24 * 60 * 60 * 1000);
	...
	await createAuditLog({
		...
		description:
			newStatus === UserStatus.SUSPENDED ? "User account suspended" : "User account activated",
