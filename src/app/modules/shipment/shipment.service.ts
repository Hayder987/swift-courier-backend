...
import {
  ...
  onboardingAuditOldDeadline,
} from "../../utils/comon.utils";
...
const createShipment = async (
  ...
  const pickupLocation = await reverseGeocode(
    Number(pickupLat),
    Number(pickupLng),
  );
  ...
  const pickUpZoneInfo = await getZoneInfo(
    Number(pickupLat),
    Number(pickupLng),
  );
  ...
  if (!pickUpZoneInfo) {
    throw new AppError(
      httpStatus.NOT_FOUND,
      "There is no Pickup Zone found for this location!",
    );
  }
  ...
  if (!deliveryZoneInfo) {
    throw new AppError(
      httpStatus.NOT_FOUND,
      "There is no Delivery Zone found for this location!",
    );
  }
  ...
  try {
    ...
    if (cloudinaryResult.public_id) {
      try {
        ...
        console.error(
          "Failed to delete Cloudinary image after transaction failure:",
          deleteError,
        );
      }
    }
    ...
    async (tx) => {
      ...
      const statusFlow: Record<ShipmentStatus, ShipmentStatus[]> = {
        [ShipmentStatus.CREATED]: [
          ShipmentStatus.READY_FOR_PAYMENT,
      ...
      if (payload.status === ShipmentStatus.OUT_FOR_DELIVERY) {
        if (!isExists.deliveryCourierId) {
          throw new AppError(
            httpStatus.BAD_REQUEST,
            "This Shipment Has No Delivery Courier",
          );
        }
    ...
    async (tx) => {
      ...
      if (payload.status === ShipmentStatus.PICKED_UP) {
        if (!isExists.pickupCourierId) {
          throw new AppError(
            httpStatus.BAD_REQUEST,
            "This Shipment Has No Pickup Courier",
          );
        }
        ...
        if (isExists.status === ShipmentStatus.PICKED_UP) {
          throw new AppError(
            httpStatus.BAD_REQUEST,
            `Shipment is already ${isExists.status}`,
          );
        }
...
// assign courier
const assignCourierOnShipment = async (
  user: IReqUserPayload,
  shipmentId: string,
) => {
  const transactionResult = await prisma.$transaction(
    async (tx) => {
      ...
      if (isExists.status !== ShipmentStatus.PENDING) {
        throw new AppError(
          httpStatus.BAD_REQUEST,
          "Only Pending Shipment Can be Assign!",
        );
      }
      ...
      if (!isExists.pickupZoneId || !isExists.deliveryZoneId) {
        throw new AppError(
          httpStatus.BAD_REQUEST,
          "Pickup and Delivery Zone are required!",
        );
      }

      const picupZoneRandomCourier = await getRandomAvailableCourier(
        isExists.pickupZoneId,
      );

      const deliveryZoneRandomCourier = await getRandomAvailableCourier(
        isExists.deliveryZoneId,
      );
