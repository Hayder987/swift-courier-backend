...
export const sendTemplateEmail = async <T extends Record<string, unknown>>(
	...
	const templatePath = path.join(process.cwd(), `src/app/templates/${payload.templateName}.ejs`);

	const html = await ejs.renderFile(templatePath, payload.data as unknown as ejs.Data);
